  ...
  update = function(_, opts)
    ...
    elseif string.match(wezterm.target_triple, 'linux') ~= nil then
      ...
      for v in result:gmatch('%d+') do
        table.insert(fields, tonumber(v))
      end
      if #fields >= 8 then
        local idle_now = fields[4] + fields[5]  -- idle + iowait
        local total_now = 0
        for i = 1, #fields do total_now = total_now + fields[i] end

        if last_sample and last_sample.total then
          local idle_delta = idle_now - last_sample.idle
          local total_delta = total_now - last_sample.total
        ...
        last_sample = { idle = idle_now, total = total_now }
      else
        cpu = 0
      end
    else
